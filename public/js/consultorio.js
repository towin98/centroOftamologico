...
document.addEventListener("click", async e => {
    if (e.target.matches("#btn-editar-consultorio")) {
        ...
        try {
            let res = await fetch(
                    "consultorio/" + e.target.dataset.id + "/edit"
                ), //Asunto-cita/{Asunto_citum}/edit
                data = await res.json();
            console.log(data);
    ...
    if (e.target.matches("#btn-eliminar-consultorio")) {
        ...
        if (result.isConfirmed === true) {
            try {
                ...
                let res = await fetch(
                    "consultorio-eliminar/" + e.target.dataset.id,
                    {
                ...
                if (res.status === 500) {
                    alert(
                        "Este registro no puede eliminarlo porque el consultario está asignado a un turno"
                    );
                }
